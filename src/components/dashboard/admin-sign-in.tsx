...
export function AdminSignIn({ t }: AdminSignInProps) {
  ...
  return (
    <div className="relative z-10 mx-auto grid min-h-[calc(100vh-6rem)] w-full max-w-lg place-items-center px-5 py-10 sm:px-8">
      <section className="w-full rounded-2xl border border-[var(--color-border)] border-t-4 border-t-[var(--color-gold)] bg-[var(--color-surface)] p-6 shadow-[0_20px_54px_rgba(23,26,25,0.1)] sm:p-8">
        <div className="flex items-center gap-3">
          ...
          <p className="text-xs font-bold text-[var(--color-green)]">{t("dashboard.auth.eyebrow")}</p>
        </div>
        <h1 className="mt-4 text-3xl font-bold text-[var(--color-ink)]">{t("dashboard.auth.title")}</h1>
        ...
        <form className="mt-6 grid gap-4" onSubmit={handleSubmit}>
          <label className="grid gap-2 text-sm font-bold text-[var(--color-ink)]">
            ...
            <input
              ...
              className="h-12 rounded-xl border border-[var(--color-border)] bg-[var(--background)] px-4 font-medium outline-none transition focus:border-[var(--color-gold)]"
          ...
          <button
            className="h-12 rounded-xl border border-[var(--color-green)] bg-[var(--color-green)] px-5 text-sm font-bold text-white transition hover:border-[var(--color-ink)] hover:bg-[var(--color-ink)] disabled:cursor-wait disabled:opacity-70"
